...
# To simplify the problem further, the GPU would
# like to remove any particles that collide.
# Particles collide if their positions ever
# exactly match. Because particles are updated
# simultaneously, more than two particles can
# collide at the same time and place. Once
# particles collide, they are removed and cannot
# collide with anything else after that tick. We
# need to know which particle will stay closest to
# position <0,0,0> in the long term. Measure this
# using the Manhattan distance, which in this
# situation is simply the sum of the absolute
# values of a particle's X, Y, and Z position.
#
# How many particles are left after all collisions
# are resolved?
...
if __name__ == '__main__':
   ...
   for _ in range(1000):
      ...
      # Sort the particles .
      new_particles = sorted(new_particles, key=lambda x: x[1])

      # Remove particles at same locations.
      particles = list()
      collision = False
      for i in range(len(new_particles) - 1):
         if new_particles[i][1] == new_particles[i+1][1]:
            collision = True
         elif collision:
            collision = False
         else:
      ...
      if not collision:
         particles.append(new_particles[-1])

   # Display the remaining number of particles.
   print("Remaining number of particles = " + str(len(particles)))
